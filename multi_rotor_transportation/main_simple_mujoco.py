...
class PayloadControlMujocoNode(Node):
    def __init__(self):
        ...
        pos_0 = np.array([0.0, 0.0, 0.65], dtype=np.double)
        ...
        pos_quad_0 = np.array([0.0, 0.0, 1.70], dtype=np.double)
        ### Linear velocity of the sytem respect to the inertial frame
        ...
        quat_quad_0 = np.array([0.7, 0.0, 0.0, 0.7])
    ...
    def callback_get_odometry_payload(self, msg):
        ...
        rotational = R.from_quat([msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z, msg.pose.pose.orientation.w])
    ...
    def quadrotor_acceleration_c(self):
        ...
        linear_acceleration = -(1/(self.mass))*t_1_cmd*n_p - self.gravity*self.e3 - ((self.mass_quad*self.length)/(self.mass + self.mass_quad))*(cross_angular_payload.T@cross_angular_payload)*n_p 
